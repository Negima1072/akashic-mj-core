...
import type { Menzi, Pai, Paizi } from "./data";
...
export class Shoupai {
  ...
  static valid_mianzi(m: Menzi): Menzi | null {
    ...
    if (h.match(/^[mpsz](\d)\1\1[\+\=\-]\1?$/)) {
      ...
      const hongpai = m.match(/0/);
      const nn = h.match(/\d/g).sort();
  ...
  static fromString(paistr?: Paizi): Shoupai {
    const fulou = paistr.split(",");
    const bingpai = fulou.shift();

    let qipai: string[] = bingpai.match(/_/g) || [];
    for (const suitstr of bingpai.match(/[mpsz]\d+/g) || []) {
      const s = suitstr[0];
    ...
    const zimo = (qipai.length - 2) % 3 === 0 && qipai.slice(-1)[0];
  ...
  toString(): Paizi {
    ...
    for (const s of ["m", "p", "s", "z"]) {
      ...
      if (suitstr !== "_") {
        const bingpai = this._bingpai[suitstr];
  ...
  gang(m: Menzi, check?: boolean): this {
    ...
    if (m.match(/\d{4}$/)) {
      ...
      const m1 = m.slice(0, 5);
      const i = this._fulou.findIndex((m2) => m1 === m2);
  ...
  get_dapai(check?: boolean): Pai[] | null {
    ...
    if (!this._lizhi) {
      for (const s of ["m", "p", "s", "z"]) {
        const suitstr = s as keyof Bingpai;
        if (suitstr !== "_") {
          const bingpai = this._bingpai[suitstr];
          for (let n = 1; n < bingpai.length; n++) {
  ...
  get_gang_mianzi(p?: Pai): Menzi[] | null {
    ...
    if (p) {
      ...
      const p = this._zimo.replace(/0/, "5");
